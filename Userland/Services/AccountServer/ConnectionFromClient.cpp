...
ErrorOr<void> ConnectionFromClient::update_user_settings(Core::Account target_user, String shell, String gecos, gid_t primary_gid, Vector<gid_t> secondary_gids)
{
    ...
    // todo: for each group in secondary_gids, ensure user is added to it.
    //       for each group that *isn't*, make sure the user is removed from it.
    //       sync each group.
    dbgln("user updated except for the secondary groups: {}", secondary_gids.size());
