...
void UserSettingsWidget::flush_staged_settings(StringView username)
{
    dbgln("flushing staged settings for {}...", username);

    auto query = m_staged_settings.get(username);
    if (query.has_value()) {
        auto const& settings = query.value();
        dbgln("{}", settings.gecos);
    }
...
void UserSettingsWidget::on_properties_click()
{
    ...
    for (auto const& account : m_accounts) {
        if (account.uid() == uid) {
            ...
            properties->on_settings_applied = [this, username = account.username(), properties = properties.ptr()] {
                if (properties->is_modified())
                    flush_staged_settings(username);
            };
