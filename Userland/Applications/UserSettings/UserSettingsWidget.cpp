    ...
    : m_accounts(accounts)
{
    ...
    m_properties_button->on_click = [this](auto) {
        ...
        for (auto const& account : m_accounts) {
            if (account.uid() == uid) {
                auto properties = MUST(GUI::SettingsWindow::create(String::formatted("{} Properties", account.username())));
                MUST(properties->add_tab<UserPropertiesGeneralTab>("General"sv, "general"sv, account));
                MUST(properties->add_tab<UserPropertiesPasswordTab>("Password"sv, "password"sv, account));
                MUST(properties->add_tab<UserPropertiesGroupsTab>("Groups"sv, "groups"sv, account));
                ...
                properties->on_settings_applied = [this, properties = properties.ptr()] {
                    auto general = static_cast<UserPropertiesGeneralTab*>(properties->get_tab("general"sv).value().ptr());
                    auto general_settings = general->get_settings();

                    dbgln("{}", general_settings.default_shell);
