...
UserSettingsWidget::UserSettingsWidget(Vector<Core::Account> const& accounts, RefPtr<ConnectionToServer> server)
    : m_accounts(accounts)
    , m_settings_server(server)
{
    ...
    m_change_root_password->on_click = [this](auto) {
        (void)m_settings_server->try_request_password_change(0, &settings_window());
    };
