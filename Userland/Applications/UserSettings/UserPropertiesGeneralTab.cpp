...
UserPropertiesGeneralTab::UserPropertiesGeneralTab(Core::Account const& account)
    : m_account(account)
{
...
UserPropertiesGeneralTab::Settings UserPropertiesGeneralTab::get_settings() const
{
    Settings result;

    result.default_shell = m_default_shell->text();
    result.full_name = m_full_name->text();
    result.contact = m_contact->text();
    result.office_tel = m_office_tel->text();
    result.home_tel = m_home_tel->text();
    result.other_info = m_other_info->text();
...
{
    dbgln("general tab saving settings");
}
