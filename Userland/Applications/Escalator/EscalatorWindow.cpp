...
EscalatorWindow::EscalatorWindow(StringView description, Vector<StringView> command, Core::Account current_user, bool preserve_env)
    : m_command(command)
...
{
    auto executable_path = Core::File::resolve_executable_from_environment(m_command[0]);

    if (executable_path.has_value())
        return GUI::FileIconProvider::icon_for_executable(executable_path.value());
